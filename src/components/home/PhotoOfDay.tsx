...
const PhotoOfDay = () => {
  ...
  return (
    <div>
      <h2 className="font-bold text-base mb-2 border-b-2 border-foreground pb-1">Фото дня</h2>
      <div className="relative news-card bg-card rounded-md overflow-hidden">
        <img src={photos[idx].img} alt={photos[idx].caption} className="w-full h-72 sm:h-[420px] object-cover" />
        <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-foreground/85 via-foreground/40 to-transparent p-4">
          <p className="text-primary-foreground text-sm font-medium">{photos[idx].caption}</p>
        </div>
        <button
        ...
        <div className="absolute top-3 right-3 bg-foreground/50 text-primary-foreground text-xs px-2 py-0.5 rounded font-mono">
          {idx + 1} / {photos.length}
        </div>
      </div>
    </div>
